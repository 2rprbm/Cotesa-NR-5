...
var refresh_objects_listeners = function () {
  ...
  $.each($(".tabs"), function (index, element) {
    let tabs_id = "tab-" + index;
    let ul_template = "<ul class='nav nav-tabs'></ul>";
    let li_template = "<li class='nav-item'></li>";

    let tab_header_container = $(this).find(".tab-items");
    let tabs_headers = $(tab_header_container).find("a");
    let tabs_body = $(this).find(".tab-pane");

    $(tabs_headers[0]).addClass("active");
    $(tabs_body[0]).addClass("active");
    $(tabs_headers).wrapAll(ul_template);
    $.each(tabs_headers, function (i, el) {
      let th_id = tabs_id + "-item-" + i;
      $(el).addClass("nav-link");
      $(el).attr("href", "#" + th_id);
      $(el).attr("data-toggle", "tab");
      $(el).wrap(li_template);
      $(tabs_body[i]).attr("id", th_id);
    });
    //
  });
  ...
  $.each(
    $(".jesus_slider_no_arrow_image_background"),
  ...
  function pick_another_question(
    ...
    $(title_container).html(round_title);
    for (var i in round_answers) {
      $(choices_container[i]).html(round_answers[i]["Title"]);
    }
    return round_answers;
  }

  $.each($(".close-tab-button"), function (index, element) {
    $(this).on("click", function () {
      window.location.href =
        "https://institutoembelleze.instructure.com/courses/49/assignments/347?module_item_id=959";
    });
  });
    ...
    for (var i = 0; i < lss_settings["answers"]; i++) {
      ...
      $(input_answer).on("click", function () {
        if (!processing_next_round) {
          ...
          $("html, body").animate(
            { scrollTop: game_box.offset().top - offset_top - margin_top },
            200
          );
          $(question_box).hide();
          $(julia_waiting).hide();
  ...
  // LSS NO ANIMATED QUIZ
  $.each($(".lss_quiz_no_animation"), function (index, element) {
    let base_url = "pages/extras/";
    let json_db_name = $(this).attr("database-name");
    let url = base_url + json_db_name + ".json";
    let _this = $(this);
    $.ajax({
      dataType: "json",
      url: url,
      success: function (data) {
        let database_json = data;
        let lss_settings = database_json["settings"];
        let lss_database = database_json["questions"];
        let game_rules = database_json["gamemode"];
        let score = 0;
        ...
        let question_box = _this.find(".questionsBox");
        let game_box = _this.find("#game_box");
        let choices_wrapper = _this.find("#choices_group");
        let progress_led = _this.find("#progress_lettering");
        let title_container = _this.find("#lettering");
        let choices_container = [];
        let feedback_rule = game_rules["feedback_rule"];
        let feedback = game_rules["feedbacks"];
        let feedback_box = _this.find("#feedbackBox");
        let feedback_lettering = _this.find("#feedback_lettering");
          ...
          $(input_answer).on("click", function () {
            if (!processing_next_round) {
              ...
              setTimeout(function () {
                ...
                } else if (feedback_rule == "feed-and-go") {
                  $(question_box).hide();
                  $(feedback_box).show();
                  score += round_answers[choice_number]["Score"];
                  let last_choice = round_answers[choice_number]["Score"] == 1;
                  if (current_question < max_questions) {
                    if (last_choice) {
                      //RIGHT ANSWER FEEDBACK
                      $(feedback_lettering).html(
                        lss_database[current_question]["Feedback-certo"]
                      );
                      ...
                      if (current_question < max_questions) {
                        ...
                        progress_led.text(
                          current_question + 1 + message_suffix
                        );
                        $(progress_led).css({
                          width:
                            (current_question + 1) * quantum_question + "%",
                        });
                      }
                    } else if (already_answered) {
                      //WRONG FEEDBACK AND KEEP GOING
                      $(feedback_lettering).html(
                        lss_database[current_question]["Feedback-errado"]
                      );
                      already_answered = false;
                      current_question++;
                      if (current_question < max_questions) {
                      ...
                      //WRONG FEEDBACK AND LAST CHANCE
                      $(feedback_lettering).html(
                        lss_database[current_question]["Feedback-chance"]
                      );
        ...
        // Game Pre-Setup
        let button_continue = _this.find("#button_continue");
        $(button_continue).on("click", function () {
          if (current_question < max_questions) {
            ...
            $(game_box).hide();
            _this.find("#feed_and_go_final_screen").show();
            save_score(score);
          }
        });
